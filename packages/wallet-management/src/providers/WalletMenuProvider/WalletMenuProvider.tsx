...
export const WalletMenuProvider: React.FC<PropsWithChildren> = ({
  ...
  const [options, setOptions] = useState<WalletMenuOpenOptions>()
  ...
  const openWalletMenu = useCallback((nextOptions?: WalletMenuOpenOptions) => {
    ...
    <WalletMenuContext.Provider value={context}>
      ...
      <I18nProvider locale={locale}>
        <WalletMenuModal open={open} onClose={closeWalletMenu}>
          <WalletMenuContent onClose={closeWalletMenu} options={options} />
        </WalletMenuModal>
      </I18nProvider>
