...
export class EVMStepExecutor extends BaseStepExecutor {
  ...
  checkClient = async (step: LeapSwapStepExtended, process?: Process) => {
    ...
    if (
      ...
      this.statusManager.updateProcess(step, processToUpdate.type, 'FAILED', {
        error: {
          code: LeapSwapErrorCode.WalletChangedDuringExecution,
      ...
      throw await parseEVMErrors(
        new TransactionError(
          LeapSwapErrorCode.WalletChangedDuringExecution,
          errorMessage
        ),
        step,
        process
    ...
    try {
      ...
      if (!step.transactionRequest) {
        throw new TransactionError(
          LeapSwapErrorCode.TransactionUnprepared,
