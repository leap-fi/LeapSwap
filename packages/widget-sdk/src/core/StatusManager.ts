import type { ChainId, LeapSwapStep } from '@leapswap/widget-types'
...
export class StatusManager {
   ...
   */
  initExecutionObject = (step: LeapSwapStepExtended): Execution => {
    if (!step.execution) {
      step.execution = {
        status: 'PENDING',
        process: [],
   ...
   */
  removeProcess = (step: LeapSwapStepExtended, type: ProcessType): void => {
    if (!step.execution) {
      throw new Error("Execution hasn't been initialized.")
    }
  ...
  updateStepInRoute = (step: LeapSwapStep): LeapSwapStep => {
