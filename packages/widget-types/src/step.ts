...
export type StepInformation = StepInformationBase & {
  ...
  integrator?: string
  /** API field name kept as-is (upstream LeapSwap / LiFi-style payload). */
  relatedLeapSwapStep?: string[]
}
...
export interface LeapSwapStep extends Omit<Step, 'type'> {
  type: 'swap' | 'bridge' | 'custom' | 'leapswap'
  includedSteps: Step[]
}
