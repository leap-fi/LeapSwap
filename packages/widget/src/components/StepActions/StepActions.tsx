...
import type { LeapSwapStep, StepExtended } from '@leapswap/widget-sdk'
import { isGaslessStep } from '@leapswap/widget-sdk'
    ...
    <Box {...other}>
      ...
      >
        <Badge
          ...
          badgeContent={<SmallAvatar src={LeapSwapLogo} />}
        >
          <StepAvatar
            variant="circular"
            src={toolDetails.logoURI ?? LeapSwapLogo}
        ...
        >
          ...
          >
            {toolDetails.name?.includes('LeapSwap') ||
            toolDetails.name?.includes('LeapSwap')
              ? toolDetails.name
              : t('main.stepDetails', {
...
export const CustomStepDetailsLabel: React.FC<StepDetailsLabelProps> = ({
  ...
  const toolDetails =
    subvariant === 'custom' &&
    (step as unknown as LeapSwapStep).includedSteps?.length > 0
      ? (step as unknown as LeapSwapStep).includedSteps.find(
          (step) => step.tool === 'custom' && step.toolDetails.key !== 'custom'
        )?.toolDetails || step.toolDetails
      : step.toolDetails
